...
class textsearch(object):
    ...
    def create_hash(self):
        ...
        if self.create_histo:
            for k_, v_ in sorted(
                    ast.literal_eval(json.dumps(Counter(' '.join(self.flatten(data, [])).split(' ')))).iteritems(),
                    key=lambda k_, v_: (v_, k_), reverse=True):
    ...
    def lookup(self, keyword, individual=False):
        yield_docs = lambda x: {'index': x, 'doc': self.dataset[x][1],
                                'doc_id': self.dataset[x][0]} if self.index_present else {'index': x,
                                                                                          'doc': self.dataset[x]}
